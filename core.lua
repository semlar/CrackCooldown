...
local function animate(button)
	if not SETTINGS.glint or not button:IsVisible() then
		return
	end
...
local function ActionTimer(parent)
	...
	timer.action =
		parent:GetAttribute("action") or (parent.GetPagedID and parent:GetPagedID()) or
		(parent.CalculateAction and parent:CalculateAction())
...
hooksecurefunc(
	getmetatable(CreateFrame("checkbutton", nil, nil, "SecureActionButtonTemplate")).__index,
	"SetAttribute",
	function(button, attribute, slot)
		--if attribute ~= 'action' or not HasAction(slot) then return end
		if attribute ~= "action" or not slot or type(slot) ~= "number" or not HasAction(slot) then
			return
		end
...
hooksecurefunc(
	"UseAction",
	function(slot1, target, mouseButton)
...
local function SetTimerTime(timer, left, color)
	...
	if not timer:IsVisible() or width <= 0 or left <= 0 then
		timer.text:SetText("")
		return
	end
	if timer.border and color then
		if IsUsableAction(timer.action) and color == SETTINGS.colors.aura then
			timer.border:SetVertexColor(
				SETTINGS.colors.aura.r,
				SETTINGS.colors.aura.g,
				SETTINGS.colors.aura.b,
				SETTINGS.colors.aura.a
			)
		else
			timer.border:SetVertexColor(
				SETTINGS.colors.real.r,
				SETTINGS.colors.real.g,
				SETTINGS.colors.real.b,
				SETTINGS.colors.real.a
			)
		end
	end
	if not color then
...
local function updateAuras()
	...
	local unit =
		(UnitExists("target") and (UnitIsPlayer("target") or UnitCanAttack("player", "target"))) and "target" or "player"
	if UnitIsFriend("player", unit) then
		...
		for i = 1, 40 do
			...
			if caster == "player" and expires > 0 and SpellIsSelfBuff(spellID) then
				...
				if MisnamedAuras[name] then
					name = MisnamedAuras[name]
				end
				if (auras[name] and expires > auras[name]) or not auras[name] then
					auras[name] = expires
				end
	...
	function(self, e)
		...
		if ts >= 0.1 then
			...
			for button, timer in pairs(actions) do
				...
				local slot =
					button:GetAttribute("action") or (button.GetPagedID and button:GetPagedID()) or
					(button.CalculateAction and button:CalculateAction())
				timer.action = slot
				if slot and HasAction(slot) and button:IsVisible() then
					...
					end -- tainting the global underscore, could use select() here
					if btype == "item" then
...
local function CreateInterfacePanel() -- TODO: Clean this up
	...
	local function defaultCallback(restore)
		SETTINGS.colors.default.r, SETTINGS.colors.default.g, SETTINGS.colors.default.b, SETTINGS.colors.default.a =
			colorCallback(restore)
		defaultTimer.text:SetTextColor(
			SETTINGS.colors.default.r,
			SETTINGS.colors.default.g,
			SETTINGS.colors.default.b,
			SETTINGS.colors.default.a
		)
	end

	local function realCallback(restore)
		SETTINGS.colors.real.r, SETTINGS.colors.real.g, SETTINGS.colors.real.b, SETTINGS.colors.real.a =
			colorCallback(restore)
		realTimer.text:SetTextColor(
			SETTINGS.colors.real.r,
			SETTINGS.colors.real.g,
			SETTINGS.colors.real.b,
			SETTINGS.colors.real.a
		)
		realTimer.border:SetVertexColor(
			SETTINGS.colors.real.r,
			SETTINGS.colors.real.g,
			SETTINGS.colors.real.b,
			SETTINGS.colors.real.a
		)
		energyTimer.border:SetVertexColor(
			SETTINGS.colors.real.r,
			SETTINGS.colors.real.g,
			SETTINGS.colors.real.b,
			SETTINGS.colors.real.a
	...
	local function energyCallback(restore)
		SETTINGS.colors.energy.r, SETTINGS.colors.energy.g, SETTINGS.colors.energy.b, SETTINGS.colors.energy.a =
			colorCallback(restore)
		energyTimer.text:SetTextColor(
			SETTINGS.colors.energy.r,
			SETTINGS.colors.energy.g,
			SETTINGS.colors.energy.b,
			SETTINGS.colors.energy.a
		)
		energyTimer.border:SetVertexColor(
			SETTINGS.colors.real.r,
			SETTINGS.colors.real.g,
	...
	local function auraCallback(restore)
		SETTINGS.colors.aura.r, SETTINGS.colors.aura.g, SETTINGS.colors.aura.b, SETTINGS.colors.aura.a =
			colorCallback(restore)
		auraTimer.text:SetTextColor(
			SETTINGS.colors.aura.r,
			SETTINGS.colors.aura.g,
			SETTINGS.colors.aura.b,
			SETTINGS.colors.aura.a
		)
		auraTimer.border:SetVertexColor(
			SETTINGS.colors.aura.r,
			SETTINGS.colors.aura.g,
			SETTINGS.colors.aura.b,
			SETTINGS.colors.aura.a
	...
	glintCheckBox:SetScript(
		"OnClick",
		function(self)
			SETTINGS.glint = self:GetChecked() and true or false
		end
	)
